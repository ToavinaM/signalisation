...
public class ImpFrontRepository implements RepositoryRegion{
    ...
    public List<Signal> signalRegions(String idRegion) throws Exception {
     String sign = "select  s.*, t.nom nomSignal, r.nom nomRegion from signal s inner join  typeSignal  t on s.idtypesignal=t.idtypesignal inner join region r on s.idRegion=r.idRegion where s.lastStatus != 'En attente' and  s.idRegion="+idRegion;
